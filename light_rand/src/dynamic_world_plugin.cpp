...
public:
 ...
 void RandomiseModelFriction()
  ...
  void RandomiseWorldModels()
  {
    ...
    for (auto const& x : this->modelIDToName)
    {
        ...
        if (!this->check_if_vector_contains_string(this->model_to_ignore_randcolours,model_name))
        {
          ...
          result = this->ChangeModelColour(model_name);
        }
    }
  ...
  {
    // ROS_INFO("Start OutputWorldModelsData...");
  ...
  bool ChangeModelColour(std::string model_name)
  {  
    ...
    // "base","right_l0","right_l1","right_l2","right_l3","right_l4", "right_l5",
    // "right_l6","right_gripper_l_finger","right_gripper_r_finger"}; // right_l5 is not available 

    std::vector<std::string> visuals {
    "base", "head",  "right_l5",
    "right_gripper_l_finger","right_gripper_r_finger"}; // right_l5 is not available 


    for (auto link : model->GetLinks())
    {

        double r = getRandomDouble(0.0,0.5);
        double g = getRandomDouble(0.0,0.5);
        double b = getRandomDouble(0.0,0.5);
        double a = getRandomDouble(0.8,1.0);
        gazebo::common::Color newColor(r, g, b, a);
      ...
      // if link->GetName().c_str()


      if (linkSDF->HasElement("visual"))
      {

        if (std::find(visuals.begin(), visuals.end(), temp) != visuals.end()) continue;
    ...
    const gazebo::common::Color & specular)
{
  ...
  bool ChangeLight(std::string light_name)
  {
    ...
    float diffuse_R = RandomFloat(0.0, 0.5);
    float diffuse_G = RandomFloat(0.0, 0.5);
    float diffuse_B = RandomFloat(0.0, 0.5);
    float diffuse_A = RandomFloat(0.0, 0.5);

    float specular_R = RandomFloat(0.0, 0.5);
    float specular_G = RandomFloat(0.0, 0.5);
    float specular_B = RandomFloat(0.0, 0.5);
    float specular_A = RandomFloat(0.0, 0.5);
